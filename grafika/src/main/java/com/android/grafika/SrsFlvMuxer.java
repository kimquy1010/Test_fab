...
public class SrsFlvMuxer {
    ...
    private boolean connect(String url) {
        ...
        mVideoSequenceHeader = null;
        mAudioSequenceHeader = null;
        Log.d(TAG, "Check connection: " + connected);
        return connected;
    }
                ...
                while (!Thread.interrupted()) {
                    ...
                    while (!mFlvTagCache.isEmpty()) {
                        ...
                        if (frame.isSequenceHeader()) {
                            Log.d(TAG, "Header frame");
                            if (frame.isVideo()) {
                                Log.d(TAG, "Have video frame");
                                mVideoSequenceHeader = frame;
                                sendFlvTag(mVideoSequenceHeader);
                            } else if (frame.isAudio()) {
                                Log.d(TAG, "Have audio frame");
                                mAudioSequenceHeader = frame;
                                sendFlvTag(mAudioSequenceHeader);
                            }
                        } else {
                            Log.d(TAG, "Not header frame");
                            Log.d(TAG, "VideosenquenceHeader: " + mVideoSequenceHeader);
    ...
    public void writeSampleData(int trackIndex, ByteBuffer byteBuf, MediaCodec.BufferInfo bufferInfo) {
        ...
        if (bufferInfo.offset > 0) {
            Log.w(TAG, String.format("encoded frame %dB, offset=%d pts=%dms",
                    bufferInfo.size, bufferInfo.offset, bufferInfo.presentationTimeUs
            ));
        }
    ...
    private class SrsRawH264Stream {
        ...
        public void muxSequenceHeader(ByteBuffer sps, ByteBuffer pps, int dts, int pts,
            ...
            Log.i(TAG, "Test profile indication: " + profile_idc + ", level indication: " + level_idc);
            Log.i(TAG,"Test sps buffer: "+sps.get(1)+sps.get(2)+sps.get(3)+sps.get(4)+sps.get(5)+sps.get(6)+sps.get(7)+sps.get(8)+sps.get(9)+sps.get(10)+sps.get(11)+sps.get(12)+sps.get(0));
            ...
            Log.i(TAG, "Test length of pps buffer: " + sps.array().length + ", length short: " + (short) sps.array().length);
            ...
            Log.i(TAG, "Test length of pps buffer: " + pps.array().length + ", length short: " + (short) pps.array().length);

            pps_hdr.data.rewind();
            frames.add(pps_hdr);
            Log.i(TAG,"Test pps buffer: "+pps.get(1)+pps.get(2)+pps.get(3)+pps.get(0));
            // pictureParameterSetNALUnit
    ...
    private class SrsFlv {
        ...
        public void writeAudioSample(final ByteBuffer bb, MediaCodec.BufferInfo bi) {
            Log.d(TAG,"Start write audio sample");
...
//                    writeH264SpsPps(dts, pts);
            ...
            Log.d(TAG, String.format("Check pts = %d and dts = %d ", pts, dts));
            writeH264IpbFrame(ipbs, type, dts, pts);
            ipbs.clear();
        }

        private void writeH264SpsPps(int dts, int pts) {
            Log.d(TAG, "Write H264SpsPps");
        ...
        private void writeRtmpPacket(int type, int dts, int frame_type, int avc_aac_type, SrsAllocator.Allocation tag) {
            Log.d(TAG, "Start write packet to server");
